...
echo "This installer is for Ubuntu-based systems only."
echo ""
echo ""
...
echo "Cleaning up any old Docker repo files..."
sudo rm -f /etc/apt/sources.list.d/docker* /etc/apt/sources.list.d/*docker*
echo ""
...
read -p "Pearl needs this to correctly detect when Docker is running. [Y/n] " choice
if [[ "$choice" =~ ^[Yy]?$ ]]; then
