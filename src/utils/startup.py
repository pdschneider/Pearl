...
from src.connections.ollama import ollama_test, load_model, get_loaded_models
from src.utils.hardware import get_hardware_stats
from src.utils.load_settings import load_data_path, load_settings
from src.connections.docker import docker_check
from src.connections.kokoro import kokoro_test
...
def startup(globals):
    ...
    def docker_windows_check():
        """Checks if bundled docker_install.ps1 is different from user's docker_install.ps1, change if so."""
        try:
            # Read the contents of the default path and hash it
            default_docker_windows_path = load_data_path("config", "docker_install.ps1", default=True)
            with open(default_docker_windows_path, 'r', encoding='utf-8') as f:
                default_docker_windows_file = f.read()
            hashed_default_docker_windows_ps1 = hashlib.md5(default_docker_windows_file.encode()).hexdigest()
            if not hashed_default_docker_windows_ps1:
                logging.warning(f"No hash for default docker_install.ps1 found.")

            # Read the contents of the current user's path and hash it
            user_docker_windows_path = load_data_path("config", "docker_install.ps1")
            with open(user_docker_windows_path, 'r', encoding='utf-8') as f:
                user_docker_windows_file = f.read()
            hashed_user_docker_windows_ps1 = hashlib.md5(user_docker_windows_file.encode()).hexdigest()
            if not hashed_user_docker_windows_ps1:
                logging.warning(f"No has for user's docker_windows.ps1 found.")

        except Exception as e:
            logging.warning(f"Unable to hash docker_windows.ps1 due to: {e}")
            return

        # Compare hashes, remove and replace old file if different
        if hashed_default_docker_windows_ps1 != hashed_user_docker_windows_ps1:
            try:
                if os.path.isfile(load_data_path("config", "docker_install.ps1")):
                    logging.debug(f"Hash mismatch. Removing old docker_install.ps1...")
                    os.remove(load_data_path("config", "docker_install.ps1"))
                logging.info(f"Updating docker_install.ps1...")
                load_data_path("config", "docker_install.ps1", default=True)

            except Exception as e:
                logging.warning(f"Unable to update docker_install.ps1 due to: {e}")

    def kokoro_windows_check():
        """Checks if bundled kokoro_install.ps1 is different from user's kokoro_install.ps1, change if so."""
        try:
            # Read the contents of the default path and hash it
            default_kokoro_windows_path = load_data_path("config", "kokoro_install.ps1", default=True)
            with open(default_kokoro_windows_path, 'r', encoding='utf-8') as f:
                default_kokoro_windows_file = f.read()
            hashed_default_kokoro_windows_ps1 = hashlib.md5(default_kokoro_windows_file.encode()).hexdigest()
            if not hashed_default_kokoro_windows_ps1:
                logging.warning(f"No hash for default kokoro_install.ps1 found.")

            # Read the contents of the current user's path and hash it
            user_kokoro_windows_path = load_data_path("config", "kokoro_install.ps1")
            with open(user_kokoro_windows_path, 'r', encoding='utf-8') as f:
                user_kokoro_windows_file = f.read()
            hashed_user_kokoro_windows_ps1 = hashlib.md5(user_kokoro_windows_file.encode()).hexdigest()
            if not hashed_user_kokoro_windows_ps1:
                logging.warning(f"No has for user's kokoro_windows.ps1 found.")

        except Exception as e:
            logging.warning(f"Unable to hash kokoro_windows.ps1 due to: {e}")
            return

        # Compare hashes, remove and replace old file if different
        if hashed_default_kokoro_windows_ps1 != hashed_user_kokoro_windows_ps1:
            try:
                if os.path.isfile(load_data_path("config", "kokoro_install.ps1")):
                    logging.debug(f"Hash mismatch. Removing old kokoro_install.ps1...")
                    os.remove(load_data_path("config", "kokoro_install.ps1"))
                logging.info(f"Updating kokoro_install.ps1...")
                load_data_path("config", "kokoro_install.ps1", default=True)

            except Exception as e:
                logging.warning(f"Unable to update kokoro_install.ps1 due to: {e}")
